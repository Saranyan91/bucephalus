	...
	private void initialize() {
		...
		jssList.setModel(new AbstractListModel() {
			String[] values = new String[] {"List Jobs of Person", "List Person's Knowledge Skill", "List Skill Gap From Job", "List Required Skills For Job"
					, "List Missing Skills For Job", "List Job Profiles Person Qualify","List Job Profile with highest demand"};
			public int getSize() {
		...
		jpsList.setModel(new AbstractListModel() {
			String[] values = new String[] {"List Company Workers", "List company Workers with salary", "List Company Labor Cost(descending Order)", "List Workers in Project", "Find Job Highest Pay For Person", "List people qualified for Job Profile", "List People Missing One Skill List for JProfile", "List People Held Jobs of Jobs of JProfile"};
			public int getSize() {
				return values.length;
			}
			public Object getElementAt(int index) {
		...
		tpsList.setModel(new AbstractListModel() {
			String[] values = new String[] {"Find Course covers skill", "Find Course teach Person For job", "Find Quickest Course for Person", "Find Courseset Covers skill set", "Find Courseset teach person for job", "Find cheapest courset person for job", "List people missing least skill for JProfile", "Missing K list for JProfile", "List skill with people count in Missing one list", "List Skill with people count in missing K List", "Find Course Help Most people"};
			public int getSize() {
				return values.length;
			}
			public Object getElementAt(int index) {
		...
		tpsPerform.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (tpsList.isSelectedIndex(8)){
					String id = getDetails("ENTER JOBPROFILECODE", 
							"Enter JobProfile code\n to find people missing only one skill");
					if((id != null) && (id.length() > 0))
					{
						try {
							int input = Integer.parseInt(id);
						//Connection conn = DbConnection.getConnection(uName,pwd);
						TableInfo ti = new TableInfo(sqlconn);
						ResultSet rss = ti.ListPeopleMissingOneSkillForJProfile(input);
						String title = input + "'s Missing skill";
						TableView tv = new TableView(rss,sqlconn,title);
						tv.tableviewframe.setVisible(true);
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
