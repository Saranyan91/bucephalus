...
public class DisSemHelper implements Runnable {
	...
	public DisSemHelper(String configuratorIP, int configuratorPort,
			 int helperIndex, int helperNetworkPort) {
		super();
	...
	public static void main(String[] args) {
		if (args.length != 4) 
			{
				System.out
					.println("usage: java Helper helperIndex coordinator-ip coordinator-port-num helper-port-num");
			
				System.exit(1);
			}
			DisSemHelper helper=new DisSemHelper(args[1],Integer.parseInt(args[2]), Integer.parseInt(args[0]), Integer.parseInt(args[3]));
			helper.run();
	}
	...
	void makeConnections() throws IOException
	{
		...
		for(int i=0;i<helperCount;i++)
		{
			...
			if(i<helperIndex)
			{
				...
				k=dis.readInt();
				if(k!=-1)
					System.out.printf("%d Recieved connection from helper %d at %s:%d\n",helperIndex,k,helperAddresses[k], helperPort[k]);
			}
			else continue;
